...
void initVMA(VkInstance instance, VkPhysicalDevice physicalDevice,
    ...
    if (vmaCreateAllocator(&allocatorInfo, &g_vmaAllocator) != VK_SUCCESS) {
        throw std::runtime_error("failed to create VMA allocator!");
    }
}

void cleanupVMA() {
    if (g_vmaAllocator != VK_NULL_HANDLE) {
...
void createImageVMA(uint32_t width, uint32_t height, uint32_t arrayLayers,
    ...
    if (vmaCreateImage(g_vmaAllocator, &imageInfo, &allocInfo, &image,
                       &allocation, nullptr) != VK_SUCCESS) {
...
VkImageView createImageView(VkDevice device, VkImage image, VkFormat format,
    ...
    // viewType = MAX_ENUM 时按层数自动推断（保持既有调用行为）
    if (viewType == VK_IMAGE_VIEW_TYPE_MAX_ENUM) {
        viewType = (arrayLayers > 1) ? VK_IMAGE_VIEW_TYPE_2D_ARRAY
